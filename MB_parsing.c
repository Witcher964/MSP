...
extern uint16_t MBHWinit( uint16_t, uint8_t, uint8_t, uint8_t);        // look MB_hardware.c
...
uint16_t    Nt15_code;                      // Set in MBTimer_init() and used to catch a flow break t1.5 event
...
{
    SlaveID = mbp->id;                      // Used in MBparsing
    ...
    Nt15_code = MBHWinit(mbp->br_indx & 8, mbp->parity, mbp->numOfBits, mbp->numOfStopBits); // Look NBR[] and other tables in MB_hardware.c
 ...
 */
uint8_t MBserve( void)  // returns 1 if a new frame is parsed
{
#ifndef PARSE_IN_INT
...
__interrupt void TIMER1_A1_ISR_HOOK(void) {     // t3.5 hunting
...
__interrupt void USCI0RX_ISR(void)
{
    ...
    {
        if( cnt > Nt15_code)
        {              // t1.5 from the last 02 interrupt to begin of this symbol
            ER_frame_bad = EV_HAPPEND;      // This error will be processed later
                                  // Nothing more to do in RECEIVE state
        }

        if( MBIndx > MB_FRAME_MAX-1)
        {      // Last acceptable position is 255 = MB_FRAME_MAX-1
            ER_frame_bad = EV_HAPPEND;                  // Ignore big frames
        }
...
eLogical InvalidFrame( void)
{
    ...
    if( SlaveID != MBBuff[0] && MBBuff[0] != MB_ADDRESS_BROADCAST)
    {
...
{                         // Returns TRUE if a response is needed
    ...
    case MB_FUNC_WRITE_REGISTER:
           ...
           {                              // In this function MBIndx == 8.
               ...
               MBIndx = 4;
               *(RegsInp+RegIndx)  = MBBuff[MBIndx++]<<8;   // High, then Low byte
               *(RegsInp+RegIndx) |= MBBuff[MBIndx++]   ;   // ... are packed in a WORD

               // Make response. MBBuff are ready
               Exception = MBE_NONE;                       // OK, make CRC to MBBuff[MBIndx] and response is ready
               break;
           }
           Exception = MBE_ILLEGAL_DATA_VALUE; // PDU length incorrect
    ...
    case MB_FUNC_WRITE_MULTIPLE_REGISTERS:
        ...
        {                              // The must: MBIndx == 11, 13, 15, ...
            ...
            if( BytesN != (RegNmb << 1) || MBIndx != (9+BytesN) )
            {                // 1 reg - MBIndx=11, 2 regs - 13,... 5 regs - 19, etc.
...
inline void BeginResponse( void)
{
